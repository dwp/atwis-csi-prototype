...
(function($){
  var msMethods = {
    'init' : function(options){
      ...
      multiSelects.each(function(){
        ...
        if (ms.next('.ms-container').length == 0){
          ...
          $('label[for="' + ms.attr('id') + '"]').attr('id', ms.attr('id') + '-label');
          var container = $('<div id="ms-'+ms.attr('id')+'" class="ms-container"></div>'),
              ...
              selectableUl = $('<ul tabindex="0" role="listbox" aria-labelledby="' + ms.attr('id') + '-label" class="ms-list"></ul>'),
              selectedUl = $('<ul tabindex="0" role="listbox" aria-label="' + Drupal.t('Selected options') + '" class="ms-list"></ul>');
          ...
          var optgroupLabel = null,
              optgroupId = null,
              optgroupCpt = 0;
          ms.find('optgroup,option').each(function(i){
            if ($(this).is('optgroup')){
            ...
            if ($(this).is("option:not(option[value=''], option[value='_none'])")){
              var klass = $(this).attr('class') ? ' '+$(this).attr('class') : '';
              var label = Drupal.checkPlain($(this).text());
              var selectableLi = $('<li tabindex="-1" aria-posinset="' + i + '" role="option" aria-label="' + label + '" class="ms-elem-selectable' + klass + '" ms-value="' + $(this).val() + '">'+ label +'</li>');
    ...
    'select' : function(value, method){
      ...
      var label = Drupal.checkPlain(text);
      var selectedLi = $('<li tabindex="-1" aria-selected="true" role="option" aria-label="' + label + '" class="ms-elem-selected'+(klass ? ' '+klass : '')+'" ms-value="'+value+'">'+ label +'</li>'),
